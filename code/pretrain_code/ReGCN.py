...
class ReGCNs(nn.Module):
    def __init__(self, args):
        ...
        self.globalAvgPool_x = nn.AvgPool2d((self.args.f, self.args.drug_number), (1, 1)) 
    ...
    def forward(self, data):
        ...
        if self.args.name == 'drug':
            ...
            XM = torch.cat((x_d_dr2, x_d_di2, x_d_se2, x_d_str2, x_d_pro2), 1).t()
            ...
            x_d_fe = x_d_fe.view(self.args.d_out_channels, self.args.drug_number).t()  # 1,708,1,708 → 708(out_dim),708(drug-number)

            ##without multi-view attention
            # XM = torch.stack((x_d_dr2, x_d_di2, x_d_se2, x_d_str2, x_d_pro2),0)
            # # XM = XM.unsqueeze(0)  
            # x_d_fe = XM.mean(dim=0)
            # x_d_fe = self.f_d(x_d_fe)
            # print(x_d_fe.shape)
            ##

            x_d = x_d_fe.mm(x_d_fe.t())
            ...
            # y_p = torch.randn(self.args.protein_number, self.args.f) 
            # print('y_p0:',y_p)
            ...
            YM = torch.cat((y_p_pro2, y_p_di2, y_p_seq2, y_p_dr2), 1).t()  # torch.cat((x, x, x), 1)
            ...
            y_p_fe = y_p_fe.view(self.args.p_out_channels, self.args.protein_number).t()  # 1,1512,1,1512 → 1512,1512(pro_number) 

            ##without multi-view attention
            # YM = torch.stack((y_p_pro2, y_p_di2, y_p_seq2, y_p_dr2), 0)
            # y_p_fe= YM.mean(dim=0) 
            # y_p_fe = self.f_p(y_p_fe)  
            # # print(y_p_fe.shape)
            ##

            y_p = torch.mm(y_p_fe, y_p_fe.t())
